...
class Lemmatizer():
    ...
    with (
            open("./dicts/forms.json", encoding='utf-8') as f,
            open("./dicts/word_probs.json", encoding="utf-8") as f1,
    ...
    def __init__(self, text, method='predict'):
        ...
        self.lemmaText, self.nondict, self.cnt = \
            self.make_lemmatized_text(method)
    ...
    def lemmatize(self, word):
        ...
        :return: a tuple (word, lemma) if there is only one possible lemma
                 a tuple (word, (lemma, lemma, lemma...))
                    if there are several possible lemmas
        """
    ...
    def unknown_words(self, word, method):
        ...
        if method == 'predict':
            ...
            if closest is not None:
                try:
                    predictedLemma = max(
                                        Lemmatizer.lemmadict[closest],
                                        key=Lemmatizer.lemmaModel.get)
    ...
    def update_dict(path_to_dict, path_to_update):
        ...
        """
        with (
                open(path_to_update, 'r', encoding='utf-8') as f,
                open(path_to_dict, encoding='utf-8') as f1):
    ...
    def predict_lemmas(file1, file2, unlemmatizedCounts, threshold=0):
        ...
        :param file1: path to file to write unlemmatized word,
            the closest dictionary form and predicted lemma(s)
    ...
    def process_text(in_path):
        ...
        with (
                open(in_path, 'r', encoding='utf-8') as f,
                open(os.path.join(path, "lem_" + filename),
    ...
    def process_files(path):
        """
        Lemmatizes texts from all the files in a given directory
        and puts lemmatized texts in a new one.
...
class Edits():
    ...
    def filter_unlemmatized(self):
        ...
        [self.filteredUnlemmatized.append(key) for key, value
         in self.unlemmatized.items() if value > self.threshold]
    ...
    def levenshtein_unknown(self):
        ...
        for word in self.filteredUnlemmatized:
            ...
            if closest is not None:
                entry = (Lemmatizer.demutate(word), closest,
                         (Lemmatizer.lemmadict[closest]))
    ...
    def known_edits2(word):
        return set(e2 for e1 in Edits.edits1(word)
                   for e2 in Edits.edits1(e1) if e2
                   in Lemmatizer.wordModel.keys())

    @staticmethod
    def known(words):
        return set(w for w in words if w in Lemmatizer.wordModel.keys())

    @staticmethod
    def correct(word):
        candidates = Edits.known(Edits.edits1(word)) or Edits.known_edits2(word)
        candidates = [c for c in candidates if c[0] in Edits.vowels
                      and word[0] in Edits.vowels or word[0] in Edits.consonants
                      and word[0] == c[0]]
        if len(candidates) != 0:
...
def main():
    ...
    parser.add_argument("type", type=str, help="Specify input type:\
                        F for files/folders and S for standard input")
    parser.add_argument("input", type=str, help="Enter input text\
                        or the path to an input file/folder")
